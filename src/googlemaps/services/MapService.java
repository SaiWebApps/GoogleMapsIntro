...
public class MapService implements ConnectionCallbacks, OnConnectionFailedListener {
	...
	private void setLocationMarker(String title) {
		...
		MarkerOptions marker = new MarkerOptions().position(l).title(title);
	...
	public void onConnected(Bundle arg0) {
		...
		try {
			...
			msg = "Current Location: " + a.getAddressLine(1);
			setLocationMarker(a.getLocality() + " " + a.getPostalCode());
			zoomIntoCurrentLocation();
		} catch (IOException e) {
			msg = "Unable to fetch current location";
		}
		Toast.makeText(mapActivity.getApplicationContext(), msg, Toast.LENGTH_LONG).show();
	}
